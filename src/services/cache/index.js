...
export const delCache = async ({ model, alias = '', userId = '' }) => {
  try {
    const key = `SMAXAPP_${service}:${model}-${alias}${userId}*`
    console.log('delCache', key)
...
export const serviceCache = ({ model = '', cacheForUser = false, cacheForBiz = true, cacheLocation = false, paramDefault = {} }) => {
  return async ({ biz, viewer, query, params }, res, next) => {
    try {
      ...
      let alias = ''
      if (cacheForBiz) { alias = biz?.id || '' }
      let userId = ''
      if (cacheForUser) { userId = viewer?.id || '' }
